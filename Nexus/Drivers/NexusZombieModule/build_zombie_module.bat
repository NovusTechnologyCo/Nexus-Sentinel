...
setlocal
REM ============================================================================================
REM Build NexusZombieModule.sys -- the module that PASSES Prepare and FAILS Commit, so the
REM zombie/leak branch of NxcMapUnmap can actually be executed.
REM
...
echo === Building NexusZombieModule ===
msbuild "%ROOT%\Nexus\Drivers\NexusZombieModule\NexusZombieModule.vcxproj" /p:Configuration=Release /p:Platform=x64 /v:minimal /nologo
if errorlevel 1 ( echo ERROR: build failed & exit /b 1 )

echo.
echo === TEST MODULE OK ===
echo   %ROOT%\Nexus\Drivers\NexusZombieModule\bin\NexusZombieModule.sys
endlocal
